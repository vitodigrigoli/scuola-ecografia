...
| Relatori | ✔ fatto con `people --cols-3 --square` (docenti in `--masonry`: tag, affiliazione, relazioni; id = slug del nome), `people --compact --square` (tutor), `roster` (comitati: righe con chip-avatar); generato da `content/build-relatori.js` | relatori |
